...
## Final steps - a Case
For this project we used the HyperPixel 4.0 Square screen - plugged directly into the Pi GPIO bus. Unfortunately the default orientation of this screen means that the Pi power cable is inserted from below - and this means that the unit cannot stand up on its own. At first we tried to change the orientation of the screen in software - and although this is possible, it seems that similarly rotating the orientation of the touch screen is not so easy. In the end a case was required to lift the unit up. A bit of light engineering later and the case is finished:
